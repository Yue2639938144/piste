...
def train_epoch(model: nn.Module, 
    ...
    for batch in dataloader:
        ...
        # 前向传播
        outputs = model(hla_idx, pep_idx, hla_biochem, pep_biochem, hla_mask, pep_mask)
        preds = outputs['pred']
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="训练HLA-Pep互作预测模型")
    ...
    # 设置日志
    logger = setup_logger("train_hla_pep", os.path.join(args.log_dir, "train_hla_pep.log"))
    ...
    # 加载数据
    logger.info("加载训练数据...")
    train_df = pd.read_csv(os.path.join(args.data_dir, "hla_pep_train.csv"))
    logger.info(f"训练数据大小: {train_df.shape}")
    
    logger.info("加载验证数据...")
    val_df = pd.read_csv(os.path.join(args.data_dir, "hla_pep_val.csv"))
    logger.info(f"验证数据大小: {val_df.shape}")
    
    logger.info("加载测试数据...")
    test_df = pd.read_csv(os.path.join(args.data_dir, "hla_pep_test.csv"))
    ...
    early_stopping = EarlyStopping(
        ...
        mode='max',
        delta=0.001,
        save_path=os.path.join(args.model_dir, "hla_pep_best.pt")
    )
    ...
    # 保存最终模型和阈值
    final_model_path = os.path.join(args.model_dir, "hla_pep_final.pt")
